...
Vizi.GraphicsThreeJS.prototype.findObjectFromIntersected=function(a,b,c){if(a.data){new THREE.Matrix4;var d=b.clone();d.applyMatrix4(a.matrixWorld);return{object:a.data,point:b,hitPointWorld:d,face:c,normal:c?c.normal:null}}return a.parent?this.findObjectFromIntersected(a.parent,b,c):{object:null,point:null,face:null,normal:null}};
...
d=c.clone().sub(e).sub(f),h=c.clone().add(e).sub(f),k=c.clone().add(e).add(f),e=c.clone().sub(e).add(f),c=new THREE.Geometry;c.vertices.push(d,h,k,e);d=new THREE.Face3(0,1,2);d.normal.copy(b);d.vertexNormals.push(b.clone(),b.clone(),b.clone(),b.clone());c.faces.push(d);d=new THREE.Face3(0,2,3);d.normal.copy(b);d.vertexNormals.push(b.clone(),b.clone(),b.clone(),b.clone());c.faces.push(d);c.computeFaceNormals();c.computeCentroids();b=new THREE.MeshBasicMaterial({color:a,transparent:!0,side:THREE.DoubleSide,
...
0});this.addObject(a);this.scenes=[];this.keyFrameAnimators=[];this.keyFrameAnimatorNames=[];this.cameras=[];this.cameraNames=[];this.lights=[];this.lightNames=[];this.lightIntensities=[];this.lightColors=[]};
...
2048,d.shadowMapHeight=2048,d.target.position.set(0,0,0)),this.lights.push(a.lights[b]),this.lightNames.push(a.lights[b]._object.name),this.lightIntensities.push(a.lights[b].intensity),this.lightColors.push(a.lights[b].color.clone());else this.lights.length||(this.controllerScript.headlight.intensity=1,this.headlightOn=!0);this.scenes.push(a.scene);this.initHighlight();this.fitToScene();this.calcSceneStats()};
...
Vizi.Viewer.prototype.highlightObject=function(a){this.highlightedObject&&this.highlightedObject._parent.removeComponent(this.highlightDecoration);a&&(this.highlightDecoration=Vizi.Helpers.BoundingBoxDecoration({object:a,color:11184640}),a._parent.addComponent(this.highlightDecoration));this.highlightedObject=a};
...
Vizi.Viewer.prototype.fitToScene=function(){this.boundingBox=Vizi.SceneUtils.computeBoundingBox(this.sceneRoot);this.controllerScript.controls.userPanSpeed=1;1>this.boundingBox.max.z?(this.controllerScript.camera.near=0.01,this.controllerScript.controls.userPanSpeed=0.01):1E3<this.boundingBox.max.z&&(this.controllerScript.camera.far=2E4);var a=this.boundingBox.max.clone().add(this.boundingBox.min).multiplyScalar(0.5);this.controllerScript.center=a;1==this.scenes.length&&(a=new THREE.Vector3(0,this.boundingBox.max.y,
2*this.boundingBox.max.z),this.controllerScript.camera.position.copy(a),this.controllerScript.camera.position.z*=2,this.cameras[0].position.copy(this.controllerScript.camera.position));this.sceneRoot.map(Vizi.Object,function(a){if(a._parent){var c=Vizi.Helpers.BoundingBoxDecoration({object:a,color:65280});a._parent.addComponent(c);c.visible=this.showBoundingBoxes}});this.sceneRadius=this.boundingBox.max.clone().sub(this.boundingBox.min).length();this.gridSize=a=Math.pow(10,Math.ceil(Math.log(this.sceneRadius)/
Math.LN10));this.gridStepSize=a/100;this.createGrid()};Vizi.Viewer.prototype.calcSceneStats=function(){this.faceCount=this.meshCount=0;var a=this.sceneRoot.findNodes(Vizi.Visual),b,c=a.length;for(b=0;b<c;b++){var d=a[b].geometry;this.faceCount+=d.faces?d.faces.length:d.attributes.index.array.length/3;this.meshCount++}this.sceneStats.meshCount=this.meshCount;this.sceneStats.faceCount=this.faceCount;this.sceneStats.boundingBox=this.boundingBox;this.dispatchEvent("scenestats",this.sceneStats)};
